...
const NPM_IGNORE = fs.existsSync(".npmignore")
  ? ".npmignore"
  : "../../.npmignore";
...
const tmpCopyBundles =
  packageWithBundles && !process.argv.includes("--skipBundleCopy");
if (tmpCopyBundles) {
...
ENTRY_POINTS.filter((entryPoint) => pkgJson[entryPoint]).forEach(
  (entryPoint) => {
    pkgJson[entryPoint] = (pkgJson[entryPoint] as string).replace(
      `${buildDir}/`,
      ""
    );
  }
...
async function runPackagePrepack(packagePrepackPath: string): Promise<void> {
  ...
  if (prepack && typeof prepack === "function") {
    ...
    console.error(
      `Could not find a prepack function in ${packagePrepackPath}.`
    );
    console.error(
...
void (async () => {
  ...
  try {
    ...
    console.error(
      `Error while trying to access ${packagePrepackPath.toString()}`
    );
    process.exit(1);
  }
